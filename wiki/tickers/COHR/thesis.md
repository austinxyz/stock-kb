---
source: austin
---

# COHR — Full Investment Thesis

**Ticker**: COHR (NYSE)
**Company**: Coherent Corp. (formerly II-VI Incorporated)
**As of**: 2026-08-13 (Q4 FY2026 earnings refresh — actuals confirmed 2026-08-12 after close, sell-the-news reaction 2026-08-13)
**Price**: $327.23 (2026-08-13, -7.99% day-after reaction to Q4 print)
**52-wk range**: $84.35 – $440.00
**Market cap**: ~$64.0B
**Verdict**: ✅ **Watch → constructive entry zone** — Q4 FY26 beat on all three headline metrics (revenue $2,050M +34% YoY/+3.5% beat; non-GAAP EPS $1.74/+7.6% beat; non-GAAP GM 40.2%, first quarter clearing the 40% thesis-strength trigger). Full 30-day post-earnings estimate revision cycle confirms the beat: EPS estimates up +7% to +13% across all forward periods, median target raised $400→$420. 2026-08-18: a sector-wide AI-optics valuation reset (COHR/Lumentum/AAOI all fell together, -7% to -17%, no COHR-specific news) pushed price to $309.53 — deepest discount to PW EV in this thesis's history, none of the 6 thesis-break triggers hit, volume in-line (0.98×) not distribution
**Language**: English | [中文](thesis.zh.md)

---

## 1. Why Does This Company Exist? + Pivotal Investment Question

Coherent Corp. exists at the physical layer of the internet — it makes the glass, lasers, and electronic-photonic components that convert electrical signals to light and back, enabling data to move at scale inside data centers, between cities, and across continents. Founded in 1971 as II-VI Incorporated, the company spent five decades accumulating compound semiconductor expertise (InP, GaAs, SiC). The January 2022 acquisition of Coherent Inc. (formerly JDSU, then Viavi) created the world's #2 optical components company and positioned COHR directly in the path of the AI infrastructure build-out.

The structural shift: AI model training and inference requires orders-of-magnitude more data movement inside data centers compared to conventional cloud workloads. The hyperscalers (AWS, Google, Meta, Microsoft) are upgrading from 400G to 800G optical interconnects now, with 1.6T on the near-term roadmap. COHR is a qualified supplier to all four. Each AI GPU cluster generates demand for thousands of optical transceivers that COHR manufactures.

**Pivotal Investment Question**: *Can Coherent Corp. sustain 20%+ annual revenue growth through FY2028 — converting AI data center optical demand into margin expansion to 40%+ gross margins — while deleveraging $2.5B+ in debt, before Chinese photonic IC makers erode its 800G/1.6T competitive position?*

Current milestone status: Revenue acceleration confirmed (Q3 FY26: $1,806M GAAP, +20.4% YoY). Margin improvement confirmed — GAAP GM% 35.2% → 36.6% → 36.9% → **37.6%** over 4 quarters (non-GAAP reported ~38.5%); expansion trajectory intact. Debt paydown on track ($600M reduced since merger). Binary: Does the margin trajectory reach 40%+ by FY2027, or does commoditization halt it at 38-39%? Q4 FY26 (8/12) is the next confirmation point.

**✅ Q3 FY2026 CONFIRMED (2026-05-06/11)**: Revenue $1,806M GAAP (+1.3% beat). EPS $1.41 (+1.1% beat). GAAP GM% = 37.6% ($680M GP / $1,806M rev); non-GAAP ~38.5% per press release. Q4 guidance consensus raised to $1,978M (+29.4% YoY). Stock reached intraday ATH $384.85 on 5/11, then continued to **$440.00 all-time high** post-5/11. **Thesis confirmed.**

**⏳ POST-ATH UPDATE (2026-08-04)**: Stock pulled back -25% from $440 ATH to current $329.96 — entering the prior pullback target zone ($350–375 Batch 2 target from 5/11 plan, now slightly below). Today's +14.51% bounce from $288 suggests selling pressure subsiding ahead of 8/12. FY27E EPS estimates further revised to $8.29 (+9.2% vs 90d ago). Analyst median target upgraded to $400 (22 analysts), implying +21.2% upside. Q4 FY26 consensus: EPS $1.617, Revenue $1.981B. Beat deceleration trend (Q1 +11.3% → Q2 +6.8% → Q3 +1.1%) bears watching — market may be fully modeling the improvement.

**⏳ PRE-EARNINGS PRICE-MOVE UPDATE (2026-08-10)**: Stock swung further — rallied to $378.80 on 8/7, then dropped -14.24% today to $325.15. Q4 FY26 earnings (8/12) has NOT yet been reported — today's move is not a fundamentals event. WebSearch confirms the cause: a social-media "optics vs. memory" narrative debate over the weekend plus broad pre-earnings de-risking across optical names (COHR -14%, Lumentum -6.6%, AXTI -16.7% same day). No COHR-specific negative news found. Net effect: MA50 gap is essentially unchanged from 08-04 (-4.9% vs -5.0%) despite the intervening volatility — the stock is right back near where the 08-04 refresh left it. Thesis-break triggers remain unconfirmed. 8/12 earnings is still the decision point.

**✅ Q4 FY2026 CONFIRMED (2026-08-12/13)**: Revenue $2,050M (+34% YoY, +13% QoQ, record quarter, beat consensus $1,981M by +3.5%). Non-GAAP EPS $1.74 (beat $1.617 by +7.6%, 6th consecutive beat, magnitude re-accelerated after Q3's weak +1.1%). GAAP GM 38.5% (+277bps YoY); non-GAAP GM **40.2%** (+215bps YoY) — **first quarter to clear the 40% non-GAAP thesis-strength trigger**. Full-year FY2026 revenue $7.12B (+22.5%). Q1 FY27 guidance: revenue $2.2-2.4B, non-GAAP GM 39.5-41.5%, non-GAAP EPS $1.85-2.05 — all above prior Street consensus levels. **Stock reaction: -7.99% on 8/13** despite the beat — WebSearch confirms this is (1) sell-the-news profit-taking after the stock had already priced in strong results, (2) analyst concern that Q1 FY27 GM guidance midpoint (40.5%) implies a slowing *rate* of margin expansion relative to the accelerating revenue ramp (volume-driven margin dilution), and (3) investor concern about the scale of CapEx required to expand 800G/1.6T capacity. **None of the 6 thesis-break triggers were hit; this is a valuation/positioning event, not a fundamentals event.** Thesis confirmed and strengthened on fundamentals; price/sentiment diverged from fundamentals as flagged as a risk in §12-B (crowded positioning).

---

## 2. Annual Financial Metrics

### Income Statement (FY ending June 30, $M)

| Metric | FY2022 | FY2023 | FY2024 | FY2025 |
|--------|--------|--------|--------|--------|
| Revenue | $3,317 | $5,160 | $4,708 | $5,810 |
| Rev Growth YoY | — | +55.5%† | -8.7%‡ | +23.4% |
| Gross Profit | $1,265 | $1,618 | $1,456 | $2,043 |
| Gross Margin % | 38.1% | 31.4% | 30.9% | 35.2% |
| Operating Income | $414 | $82 | $124 | $535 |
| Operating Margin | 12.5% | 1.6% | 2.6% | 9.2% |
| Net Income (GAAP) | $235 | -$260 | -$157 | $49 |
| EPS (Diluted, GAAP) | $1.45 | -$2.93 | -$1.84 | -$0.52§ |
| Diluted Shares | 116.5M | 137.6M | 151.6M | 154.8M |

† FY2023 jump reflects first full year of II-VI + Coherent consolidated (merger closed Jan 2022).
‡ FY2024 decline driven by telecom end-market downturn (ROADM, submarine — not AI datacenter).
§ GAAP diluted EPS negative to common due to $130M annual preferred stock dividends; company earns $49M GAAP net income but preferred holders receive priority.

### Cash Flow ($M)

| Metric | FY2022 | FY2023 | FY2024 | FY2025 |
|--------|--------|--------|--------|--------|
| Operating CF | $413 | $634 | $546 | $634 |
| CapEx | -$314 | -$436 | -$347 | -$441 |
| Free Cash Flow | $99 | $198 | $199 | $193 |
| Stock-Based Comp | $73 | $149 | $126 | $160 |
| FCF Margin | 3.0% | 3.8% | 4.2% | 3.3% |

Note: FCF is thin due to the capital-intensive manufacturing model. CapEx at ~7-8% of revenue is structural. FCF improvement requires margin expansion to be material.

### Balance Sheet (June 30, 2025, $M)

| Metric | Value |
|--------|-------|
| Cash + ST Investments | $909M |
| Total Debt | $3,894M |
| Net Debt | $2,985M |
| Preferred Stock | $2,483M (Series B convertible) |
| Goodwill | $4,471M |
| Other Intangibles | $3,205M |
| Total Assets | $14,911M |
| Common Equity | $5,645M |
| Shares Outstanding | ~187.5M (diluted including preferred conversion) |
| Annual Preferred Dividends | ~$130M (drag on common EPS) |
| Debt paydown (FY2023→FY2025) | -$595M |

---

## 3. Geographic Revenue Mix

Data gap: COHR does not disclose clean geographic revenue by segment in quarterly press releases. Key known facts: US hyperscaler customers (AWS, Google, Meta, Microsoft) are estimated to represent 50%+ of Networking segment revenue. Manufacturing is geographically diversified (US, Japan, Germany, Thailand, China). Telecom revenue skews toward non-US (European and Asian carriers). Full geographic breakdown pending next 10-K filing review.

---

## 4. Revenue Mix & Business Model

### Segment Structure

| Segment | FY2025 Revenue Est. | Growth Driver | Status |
|---------|--------------------:|--------------|--------|
| Networking | ~$3,400M (58%) | 800G/1.6T transceivers for AI datacenters | Accelerating; primary growth engine |
| Materials | ~$1,400M (24%) | SiC for EV/power + compound semis | Recovering; AI chip cooling tailwind |
| Lasers | ~$1,000M (17%) | Industrial/semiconductor capital equipment | Stable; cyclical |

Note: Segment revenue split estimated from public commentary; exact split not fully disclosed quarterly.

### Business Model Mechanics

Vertically integrated manufacturer: COHR designs, fabricates, packages, and tests components in-house. This provides quality differentiation and supply security for hyperscaler customers, but requires continuous CapEx investment. Revenue model is product-based (no recurring subscription); pricing determined by volume negotiations with hyperscalers (annual or multi-year contracts). Gross margin at scale is driven by product mix (transceivers at 35-40% GM vs lasers at 30-35%).

### Customer Engagement

| Customer | Role | Status |
|----------|------|--------|
| AWS | Primary hyperscaler; 800G transceiver buyer | Production — ongoing multi-year ramp |
| Google | Primary hyperscaler; AI infrastructure optical | Production — qualified 800G and 1.6T pipeline |
| Microsoft | Primary hyperscaler; Azure AI buildout | Production — significant 800G volumes |
| Meta | Primary hyperscaler; AI cluster buildout | Production — DCN optical demand |
| ZTE / Huawei (indirect) | Telecom ROADM supplier | Reduced exposure post-export controls |

---

## 5. Competitive Moat

**What makes COHR defensible:**
- Manufacturing scale: Building a compound semiconductor fab (InP, GaAs) takes 7-10 years and billions in capital. COHR has this; most competitors do not
- Hyperscaler qualifications: Each hyperscaler has a 12-18 month qualification process for new optical module suppliers. COHR is qualified at all four major hyperscalers — this is a multi-year competitive barrier for new entrants
- Vertical integration: From wafer growth → component fabrication → module assembly → testing. Enables superior quality control versus purely fabless optical companies

**What is eroding the moat:**
- Chinese optical module makers (Innolight, Accelink, HiSilicon) have qualified at some hyperscalers and compete on price for commodity 400G and lower-tier 800G
- Co-packaged optics (CPO) technology — where optical components are integrated directly into GPUs/ASICs — could eventually reduce the standalone transceiver TAM in 3-5 years
- Intel Photonics (silicon photonics) and Broadcom (optical) are investing in integrated photonic approaches that could reduce reliance on compound semiconductor modules

**Direct Competitors**

| Company | Revenue | Strength | Weakness |
|---------|---------|---------|---------|
| Lumentum (LITE) | $2.1B | Pure optical, 65%+ growth, higher margins | Smaller scale; more telecom exposure |
| Ciena (CIEN) | $5.1B | Full optical networking systems | Systems integrator, not component maker |
| Innolight (private) | ~$1B est | Chinese cost advantage; fast-follower | Not listed; geopolitical risk cuts both ways |
| Intel Photonics | N/A | Silicon photonics IP; co-packaged future | Non-core to Intel; execution risk |
| Neophotonics | Acquired by Intel 2023 | InP expertise | Absorbed into Intel |

**Verdict**: Narrow moat. The hyperscaler qualification barrier and manufacturing scale provide 2-3 year defensibility. Beyond FY2028, silicon photonics commoditization and Chinese competition are material risks to premium margins. Monitor Chinese market share at hyperscalers as leading indicator.

---

## 6. Management & Leadership

| Name | Role | Tenure | Background |
|------|------|--------|-----------|
| David Bhatt | CEO | Since March 2023 | Previously CFO of II-VI; guided merger integration |
| Chi Shi | EVP, Technology | Long-tenured | Former II-VI CTO; compound semiconductor expert |
| Mary Jane Raymond | CFO | Since 2022 | Merger integration, debt management focus |

**Capital allocation track record**: The II-VI/Coherent merger (closed Jan 2022 for ~$6.5B enterprise value) was the defining capital decision. At time of announcement (Sept 2020), it was derided as overpriced (Coherent legacy business was declining). In retrospect, the merger positioned COHR perfectly for the AI optical buildout — the networking segment was worth acquiring for hyperscaler relationships alone. The debt load is the cost. Management has prioritized debt paydown ($600M in 2 years) and gross margin improvement over share buybacks — correct capital allocation for this phase.

**Verdict**: Management B+ — merger execution was strategically prescient even if financially dilutive initially. Debt paydown pace and margin improvement execution are key monitoring items.

---

## 7. Strategic Growth Initiatives

**Near-term (FY2026, ending June 2026):**
1. 800G transceiver ramp at all four major hyperscalers — the current primary growth driver
2. Gross margin improvement toward 37-38% (from 35.2% in FY2025) through product mix shift to higher-margin AI modules
3. Continued debt paydown; target net debt below $2.5B by June 2026
4. Q3 FY2026 earnings report (May 6, 2026 after close) — key milestone to validate acceleration

**Medium-term (FY2027-28):**
1. 1.6T transceiver volume ramp — hyperscalers are already qualifying; COHR has announced 1.6T development
2. Photonic integrated circuit (PIC) development — moving from discrete components to integrated photonic chips for higher margin
3. SiC power device ramp for AI chip cooling and EV market
4. Telecom recovery as ROADM demand returns post-trough (FY2024 was the bottom)

**Long-term (FY2029+):**
1. Co-packaged optics participation — if CPO becomes standard, COHR aims to supply the photonic chips embedded in AI chips
2. Compound semiconductor diversification beyond optical (power electronics, quantum computing components)
3. Potential preferred stock retirement if FCF improves sufficiently

---

## 8. Key Risks (Impact × Probability)

| Risk | Impact | Probability | Composite |
|------|--------|------------|-----------|
| Hyperscaler AI capex pause / CapEx slowdown | Critical | 25% | High |
| Chinese photonic module market share gain at hyperscalers | High | 40% | High |
| *(2026-08-13 update)* Margin-expansion rate decelerates from here (Q1 FY27 guide midpoint only +30bps above Q4 actual) | Medium | 35% | Medium — this is the specific concern behind the 8/13 sell-the-news reaction; not a reversal, but bears watching at Q1 FY27 print |
| Co-packaged optics disrupts standalone transceiver TAM | High | 30% | Medium-High |
| Preferred stock conversion creates dilution pressure | Medium | 60% | Medium |
| Balance sheet stress if FCF remains thin (~$200M) | Medium | 25% | Medium |
| Telecom recovery takes longer than expected | Medium | 30% | Medium |
| FX headwinds (manufacturing in EUR, JPY regions) | Low | 50% | Low |

**Most underappreciated risk**: The preferred stock ($2.48B at $345/share = 3.8% of market cap but carries $130M/year in cash dividends) structurally limits common shareholder returns until either retired or converted. If COHR raises capital or refinances preferred at unfavorable terms, it would be a meaningful negative signal. Track preferred stock balance quarterly.

---

## 9. Industry-Specific Macro Analysis

**TAM**: Optical transceiver market for data centers estimated at $12B in 2024, growing to $25-30B by 2028 (CAGR ~20-22%). Global optical components total addressable market (including telecom) is ~$30B today, growing to $60B+ by 2030 (Source: Lightcounting, Dell'Oro estimates).

**End-market dynamics**:
- AI datacenters (800G/1.6T): Fastest-growing sub-market; 40-50% annual growth in optical transceiver volumes. Hyperscalers committed to $200B+ combined 2025 CapEx.
- Telecom (ROADM, coherent long-haul): Bottomed in FY2024; gradual recovery as carriers upgrade 5G backhaul and submarine cable.
- Industrial lasers: Stable; semiconductor capital equipment spending correlates with node transitions.
- SiC (Silicon Carbide): Slower than expected due to EV demand normalization; less important for near-term thesis.

**Competitive dynamics**:
- US-China export controls (2022-2024) reduced COHR's exposure to Chinese telecom customers but also restricted Chinese competitors' access to advanced US equipment.
- The 800G market is currently supply-constrained — hyperscalers are buying all available production.
- The 1.6T market begins ramping in 2026; first mover qualifications are critical.

---

## 10. Valuation & Comparable Analysis

### Current Multiples

| Metric | Value | Context |
|--------|-------|---------|
| EV/TTM Revenue | 10.5x | Premium for AI growth story |
| EV/FY2026E Revenue | 9.5x | $6.97B consensus estimate |
| EV/FY2027E Revenue | 7.4x | $8.90B consensus estimate |
| Non-GAAP Forward P/E (FY2026) | 64.0x | $342 / $5.35 FY2026 EPS |
| Non-GAAP Forward P/E (FY2027) | 44.9x | $342 / $7.63 FY2027 EPS |
| EV/EBITDA (TTM) | 54.6x | EBITDA ~$1.21B |
| P/S (Trailing) | 10.3x | |
| FCF Yield | 0.3% | $193M / $64.7B MC — thin |

### Peer Comparison

| Ticker | Mkt Cap | Rev TTM | EV/Rev | Rev Growth | GM% | Profitable? |
|--------|---------|---------|--------|------------|-----|------------|
| COHR | $64.7B | $6.29B | 10.5x | +17.5% | 36.4% | Marginally (GAAP) |
| LITE | $66.8B | $2.11B | 31.8x | +65.5% | 37.1% | Yes |
| CIEN | $81.1B | $5.12B | 15.8x | +33.1% | 42.1% | Yes |
| VIAV | $12.4B | $1.37B | 9.1x | +42.8% | 60.4% | Yes |
| AAOI | $14.4B | $0.46B | 31.5x | +33.9% | 30.0% | No |

COHR's valuation (10.5x EV/TTM) appears reasonable vs the group, especially given its scale advantage. However, its gross margin (36.4%) and revenue growth rate (17.5%) are both in the lower half of optical peers — justifying a discount to LITE or CIEN.

### Forward Revenue Scenario → Implied Share Price

| Scenario | FY2027 Revenue | EV/Rev Multiple | Implied EV | Net Debt | Pref | Implied MC | Implied Share Price |
|----------|---------------|-----------------|-----------|----------|------|-----------|-------------------|
| Bear | $7.0B | 6x | $42B | $2.5B | $2.5B | $37B | $198 |
| Base | $9.0B | 8x | $72B | $2.0B | $2.5B | $67.5B | $360 |
| Bull | $11.0B | 10x | $110B | $1.5B | $2.0B | $106.5B | $568 |

Assumes 187.5M shares; uses net debt / pref estimates for late FY2027.

### Analyst Consensus

| Item | Value |
|------|-------|
| Number of analysts | 22 (updated 2026-08-04) |
| Recommendation | Buy (consensus) |
| Mean target | $391.45 |
| Median target | $400.00 |
| High target | $465.00 |
| Low target | $230.00 |
| Current price vs. median | **–17.5% BELOW** ($329.96 vs $400 median) |

**Notable (2026-08-04)**: Stock is now trading 17.5% BELOW analyst median target — significant discount vs the AT-consensus position in May. Raymond James raised to $435 on 7/2. The pullback to $330 has created a more favorable risk/reward: median target implies +21.2% upside. High target $465 = +41% from current price.

---

## 11. Position Building Strategy

### Scenario A (Committed — willing to hold through volatility)

**⚠️ DO NOT execute ahead of tonight's earnings.** After earnings release:

| Tranche | Trigger | Size | Form |
|---------|---------|------|------|
| 1 | Post-earnings pullback to $300-330 zone | 50% of target position | Stock |
| 2 | Stock holds $300 and re-tests $330-340 | 25% of target | Stock |
| 3 | Revenue guide raised above $1.9B for Q4 FY26 | 25% of target | Stock |

Total cap: roughly 7–8% of the speculative bucket.

### Scenario B (Recommended — current stance)

**Watch. Wait for post-earnings reaction.** The stock ran +51.6% in 3 months and is trading above analyst median target heading into a catalyst event. Three post-earnings scenarios:

1. **Strong beat + raised guidance**: Stock gaps to $360-380. Wait for consolidation. Buy only if it holds $355 and forms a 3-5 day base. Entry: $360-370 zone.
2. **In-line result**: Stock likely pulls back to $310-330 (MA50 territory by then). This is the better entry zone. Buy in $310-330 with stop at $280.
3. **Miss or guide-down**: Stock could retrace to $270-290 zone. Wait for stabilization before entering.

### Options vs Stock

At current price with earnings tonight, options are dangerous (IV will be extremely elevated pre-earnings, then crush post-earnings). Use stock only. No pre-earnings options positions.

---

## 12. BAIT Framework (Mauboussin)

### B — Behavioral

The optical sector narrative ("AI demands light, not just silicon") has become mainstream. After COHR ran +51.6% in 3 months, consensus positioning is crowded. Behavioral risk: the stock has been featured in multiple analyst upgrades at prices now below current — retail momentum chasers dominate the short-term move. **Verdict**: Overconfident short-term bulls; creates post-earnings selling risk even on a beat (sell the news).

### A — Analytical

COHR's GAAP reporting is complex (preferred dividends, D&A from $8B goodwill/intangibles, restructuring charges). Non-GAAP EPS ($1.29 in Q2 FY26) dramatically overstates cash economics vs GAAP ($-0.52 annual). The FCF ($193M on $64.7B market cap = 0.3% yield) is the honest number. Most bulls are modeling non-GAAP and not adjusting for preferred. **Verdict**: The non-GAAP/GAAP gap is analytically important; FCF is the correct lens.

### I — Informational

Hyperscaler CapEx commitments (AWS $104B, Google $75B, Meta $65B, Microsoft $80B — 2025 combined) are public knowledge. What the market doesn't fully price: the specific 800G allocation within that CapEx (COHR doesn't disclose customer-level volume), and whether COHR's Chinese competition is gaining qualification at any major hyperscaler. **Verdict**: Informational edge is hard to find here; channel checks on 800G supply chain are the premium information.

### T — Technical

**~5-6/8** Stage 2 weakened (updated 2026-08-04). Price $329.96 has breached MA50 ($347.42, price -5.0%) — the primary Stage 2 criterion now failing. However: MA150 ($309.58, price +6.6%) and MA200 ($254.95, price +29.4%) remain intact and rising strongly. ATR $30.53. The -25% drawdown from ATH $440 resolved the prior overextension concern. Today's +14.51% bounce on 1.20× volume may signal exhaustion of selling pressure. 8/12 earnings is the next binary: strong Q4 could reclaim MA50 and restore Stage 2 8/8. **Verdict**: Stage 2 temporarily weakened by MA50 breach; MA150/200 structure intact; 8/12 earnings = reclaim or breakdown catalyst.

**BAIT Overall**: Among optical sector plays in the watchlist, COHR scores highest on T (Stage 2 strong) and A (large cap, reasonable valuation vs peers), moderate on I (public information is well-priced), and worst on B (crowded positioning before earnings). Compare to NVTS: higher B and A risk (smaller, less analyzed), similar T risk.

---

## 13. Bull / Bear / Base Scenarios

| Scenario | Probability | Price Target (18m) | Δ% from $380 | Key Assumption |
|----------|------------|-------------------|-------------|----------------|
| Bull | 30% | $540 | +42% | AI capex sustained; GM reaches 40%+; 1.6T ramp ahead of schedule; debt below $2B |
| Base | 45% | $440 | +16% | Revenue per consensus ($7.05B FY26, $9.32B FY27); margins 38-39%; debt steady |
| Bear | 25% | $215 | -43% | Hyperscaler capex pause; revenue guide-down; stock re-rates to 7x NTM revenue |

**Revenue trajectories:**

| Scenario | FY2026E | FY2027E | GM% Target |
|----------|---------|---------|------------|
| Bull | $7.5B | $10.5B | 40%+ |
| Base | $7.05B | $9.32B | 38-39% |
| Bear | $6.5B | $7.0B | 35-36% |

**Probability-Weighted EV (updated 2026-05-11):**
PW EV = (0.30 × $540) + (0.45 × $440) + (0.25 × $215)
= $162 + $198 + $53.75 = **$413.75**

PW EV $413.75 vs current $329.96 = **+25.4% expected upside** (updated 2026-08-04)

Prior reference: PW EV $413.75 vs $379.69 = +9.0% upside (2026-05-11 at ATH). Now at $329.96 (post-ATH -25% pullback), the same scenario targets yield materially better risk/reward. FY2027 EPS estimates revised further to $8.29 (+9.2% vs 90d ago, up from $8.02). Scenarios remain unchanged pending Q4 FY26 actuals on 8/12.

**2026-08-10 update**: Price now $325.15 (round-tripped through $378.80 on 8/7). Scenario targets unchanged — no fundamental data to revise them on (Q4 FY26 hasn't reported). PW EV $413.75 vs $325.15 = **+27.2% expected upside**, slightly wider than 08-04's +25.4%. This is purely mechanical (price moved, scenarios didn't) and should not be read as thesis strengthening — it's noise pending the real data point on 8/12.

**2026-08-13 update — Q4 FY26 actuals in hand, scenario probabilities shift toward Bull:**
Q4 confirmed the Bull-case assumption ("GM reaches 40%+") one quarter into FY2027 territory ahead of the original 18-month scenario horizon, and FY2027 consensus EPS ($8.37) now sits above the original Base-case revenue trajectory. Probabilities re-weighted modestly toward Bull given confirmed margin breakthrough and reaccelerated beat magnitude; Bear probability trimmed given the clean beat removes near-term "guide-down" tail risk (though CapEx/margin-rate-of-change concern from today's reaction keeps Bear non-trivial):

| Scenario | Probability (was) | Probability (now) | Price Target (18m) | Key Assumption |
|----------|-------------------|--------------------|--------------------|--------------------------------|
| Bull | 30% | **35%** | $540 | 40%+ GM confirmed one quarter early; 1.6T ramp on schedule; debt below $2B |
| Base | 45% | **45%** | $440 | Unchanged — FY27 consensus revenue/margin trajectory intact |
| Bear | 25% | **20%** | $215 | Reduced — near-term guide-down risk lowered by clean Q4 beat; CapEx/margin-deceleration remains the residual bear case |

PW EV = (0.35 × $540) + (0.45 × $440) + (0.20 × $215) = $189 + $198 + $43 = **$430**

PW EV $430 vs current $327.23 = **+31.4% expected upside** — widest gap since the thesis was opened, driven by both price (post-earnings pullback) and fundamentals (confirmed 40% GM breakthrough, upgraded Bull-scenario confidence).

**2026-08-18 update**: Price $309.53 (sector-wide optics reset, -10.4%; no fundamental change). PW EV $430 vs $309.53 = **+38.9% expected upside** — new widest gap since thesis inception, purely mechanical (price moved further down, scenario inputs unchanged; estimates if anything moved up over the same 30 days).

---

## 14. Bottom Line

**1-Year View (updated 2026-08-04):**
- Base case: $440 (+33% from $330) on FY2027 consensus ($9.32B+ revenue, $8.29 EPS, ~38-39% GM). Key upside trigger: Q4 FY26 GM% ≥39% and revenue ≥$1.981B → MA50 reclaim likely → Stage 2 restored. Key downside trigger: Q4 FY2026 revenue <$1,850M or GM% declines below 37%.
- Near-term: Stock at $330 is now well below analyst median target ($400, -17.5%). 8/12 earnings is the critical binary — pass → potential +15-20% recovery; fail → test MA150 ($310) or below.

**3-Year View:**
- Bull: $550-600 (8-10x EV/FY2028 $12B revenue scenario if margins reach 40%+ and 1.6T ramps)
- Base: $440-500 (8x EV/FY2027 $9.3B revenue; debt meaningfully reduced; GM 38-39%)
- Bear: $160-215 (EV/revenue re-rates to 6x; revenue stalls at $7B; hyperscaler capex cuts)

**Portfolio Allocation (tax-advantaged bucket):**
- Batch 1 cost basis in Zone B range; current price near or slightly below entry level
- Stop: $335 (Batch 1 stop level — current price $329.96 is near this level)
- Primary monitoring: Q4 FY2026 **2026-08-12** — pass threshold GM% ≥39%, Revenue ≥$1,978M

**Verdict (updated 2026-08-04)**: Thesis intact, technical temporarily weakened. The -25% pullback from ATH $440 to $330 has occurred — exactly the zone targeted as Batch 2 entry on 5/11. PW EV $413.75 vs $329.96 = +25.4% upside, significantly better than +9% at ATH. FY27E EPS estimates upgraded to $8.29 (+9.2% vs 90d ago). Beat deceleration trend (Q3 +1.1%) is the primary fundamental concern — watch Q4 beat magnitude closely. Existing holders near stop — 8/12 earnings is a binary hold/exit decision point. New entries at current zone have improved R/R but earnings event risk is elevated.

**Verdict (updated 2026-08-10)**: **Unchanged — thesis intact, no new information.** Today's -14.24% completes a round trip through $378.80 (8/7) with no fundamental data released; WebSearch attributes it to a weekend social-media "optics vs. memory" debate plus broad pre-earnings de-risking (COHR/LITE/AXTI all fell together today). None of the 6 thesis-break triggers were hit. The net MA50 gap barely moved (-4.9% vs 08-04's -5.0%) — this was volatility, not trend change. **8/12 earnings remains the sole decision point**; do not treat today's drop as new evidence in either direction.

**Verdict (updated 2026-08-13)**: **Thesis strengthened on fundamentals; entry window opened by price action.** Q4 FY26 beat on revenue (+3.5%), EPS (+7.6%), and non-GAAP GM (40.2%, clearing the 40% thesis-strength trigger for the first time) — the binary question posed in §1 ("does GM reach 40%+?") resolved YES, one quarter ahead of the FY2027 target horizon. Beat magnitude re-accelerated (+7.6% vs Q3's weak +1.1%), addressing the deceleration concern flagged in the prior two refreshes. Despite this, the stock fell -7.99% the next day on sell-the-news dynamics and a narrower concern about the *rate* of margin expansion implied by Q1 FY27 guidance (39.5-41.5%, midpoint only +30bps above Q4's actual 40.2%) plus CapEx-scale questions for capacity expansion. **None of the 6 thesis-break triggers were hit; the reaction is a valuation/positioning event, not a fundamentals event.** Price $327.23 now sits within the Scenario B §11 "in-line/beat-then-fade" entry zone ($310-330) — PW EV $430 vs $327.23 implies +31.4% upside, the widest gap in this thesis's history. Primary remaining watch item: whether Q1 FY27 (report ~11/4) confirms the GM expansion *rate* is merely decelerating (still healthy) vs. genuinely stalling (would be a concern).

**Verdict (updated 2026-08-18)**: **Unchanged — thesis intact, technical pullback with no fundamental counterpart.** Price fell -10.4% to $309.53 in a sector-wide AI-optics reset (Coherent, Lumentum, Applied Optoelectronics all fell together -7% to -17%; no COHR-specific news identified via WebSearch), giving back most of 8/17's +8% round-trip bounce. Meanwhile the full 30-day post-Q4 estimate revision cycle turned in clean, confirming the beat: EPS estimates rose +7% to +13% across every forward period (current quarter through FY2027), and the analyst median target rose $400→$420 in the same window. Zero estimate cuts accompany today's drop. Volume 0.98× (in-line, not distribution). **None of the 6 thesis-break triggers were hit.** PW EV $430 vs $309.53 = +38.9% upside, the widest discount since this thesis opened — price and fundamentals have diverged further, not converged. Q1 FY27 (~11/4) remains the sole hard decision point on whether the GM-expansion-rate concern (raised 8/13) materializes.

---

## 15. Monitoring Checklist

### Near-Term (Next 60 days)

- [✅] **Q3 FY2026 earnings (May 6, 2026 — CONFIRMED)** — PASSED
  - EPS $1.41 vs $1.394 est: ✅ Beat (+1.1%) — deceleration noted; watch Q4
  - Revenue: ✅ ~$1,804M (beat est. $1,780M by +1.3%; confirmed via TTM inference)
  - Q4 FY26 guidance: ✅ Consensus raised to $1,978M (rev) / $1.61 EPS — strong signal
  - Stock action: ATH $364.80 (5/6) → -6.7% (5/7) → +13.25% new ATH $384.85 (5/11)
- [✅] Post-earnings gross margin: Q3 FY26 GM% ~38.5% (new high; +160bps QoQ from Q2 36.9%) ✅
- [✅] Stock holding >$300 post-earnings: ✅ Far above at $379.69
- [✅] Entry execution: Batch 1 executed @ $331.16 (5/7) in Zone B ✅
- [✅] Q4 FY2026 revenue guidance ≥$1,850M: ✅ Implied YES — consensus $1,978M
- [⚠️] Beat magnitude deceleration: Q1 +11.3% → Q2 +6.8% → Q3 +1.1%; Q4 FY26 beat% = next key read (market may be fully pricing recovery)

### Mid-Term (3-6 months)

- [✅] **Q4 FY2026 earnings — 2026-08-12 (CONFIRMED, released)**
  - Pass: Revenue ≥$1,981M (consensus), EPS ≥$1.617, gross margin ≥39% (GAAP ≥38%) — ✅ Revenue $2,050M, EPS $1.74, GAAP GM 38.5%
  - Stretch (thesis-strength trigger): GM% ≥40% non-GAAP (GAAP ≥38.5%) — ✅ **HIT: non-GAAP GM 40.2%**
  - Fail: Revenue <$1,850M or margin expansion stalls or declines → Trim/Exit — not triggered
  - Beat magnitude: Q3 was only +1.1% — Q4 re-accelerated to +7.6% ✅
- [ ] **Q1 FY2027 earnings — ~2026-11-04 (next)**
  - Pass: Revenue within/above guided $2.2-2.4B, non-GAAP GM ≥39.5% (guided low end), non-GAAP EPS ≥$1.85
  - Watch item: does non-GAAP GM confirm ≥40.5% (guided midpoint) or slip toward 39.5% (guided low end) — this is the "rate of margin expansion" question the market is pricing in post-Q4
- [ ] Net debt trajectory: Q4 FY26 actual balance sheet pending yfinance update (Q3 was ~$1.83B net debt, down from FY25 $2.985B) — confirm continued decline
- [ ] Chinese optical competitive data: Any hyperscaler disclosed switching to Innolight/Accelink for 800G?
- [ ] 1.6T transceiver customer qualification announcement (timing indicator for next upcycle)
- [ ] Preferred stock retirement or conversion update from management

### Thesis-Break Triggers

Any ONE of the following → immediate reassessment (sell or avoid):

- [ ] Revenue guide-down for two consecutive quarters (both absolute AND YoY growth decelerating below 15%)
- [ ] Gross margin declines below 34% for any quarter (commoditization signal)
- [ ] Hyperscaler CapEx guide materially cut (AWS/Google/Meta/Microsoft combined below $300B for 2026)
- [ ] Co-packaged optics (CPO) adoption by a major hyperscaler announced without COHR as a CPO supplier
- [ ] Net debt increases YoY (FCF insufficient to sustain deleveraging)
- [ ] Any confirmed loss of a top-4 hyperscaler qualification for 800G or 1.6T

### Continuing Thesis-Strength Triggers

Any ONE of the following → add to position / increase conviction:

- [✅] **Gross margin ≥40% in any fiscal quarter — HIT (Q4 FY26: non-GAAP 40.2%)**
- [ ] Net debt below $2.0B (2024 balance sheet healed) — pending Q4 balance sheet confirmation (Q3 trajectory ~$1.83B was already close)
- [ ] 1.6T production ramp announcement with named hyperscaler
- [ ] Preferred stock retirement announcement (removes $130M annual dividend drag)

---

## Sources

- Yahoo Finance (yfinance): Financial statements, price data, analyst consensus (2026-08-04)
- COHR Q3 FY2026 earnings release (May 6, 2026; public)
- Company segment disclosures from annual and quarterly reports (FY2024-FY2025)
- Lightcounting / Dell'Oro optical market forecasts (industry estimates)
- Analyst consensus from Yahoo Finance (22 analysts, 2026-08-04)
- SEPA technical analysis via local yfinance computation (2026-08-04)
- Finance-sentiment (Adanos API): X.com, Reddit, News (2026-08-04)
- Raymond James upgrade note (7/2/2026): raised to $435 from $371

- Coherent Corp. Q4 & Full Year FY2026 earnings press release (GlobeNewswire, 2026-08-12)
- WebSearch: post-earnings reaction attribution (2026-08-13) — sell-the-news, margin-guidance-rate, CapEx-scale concerns

**Data gaps**: 
- ✅ Q4 FY2026 actuals now confirmed (2026-08-12) — resolved
- Q4 FY2026 balance sheet (net debt, cash) not yet reflected in yfinance as of 2026-08-13 refresh; company press release net debt figure not captured in this pass — confirm at next refresh
- Geographic revenue breakdown not cleanly disclosed
- Customer concentration (top 3 customer revenue %) not publicly disclosed
- Chinese competitor market share at hyperscalers — channel check data needed
- GAAP vs non-GAAP GM% gap: Q4 FY26 GAAP = 38.5%, non-GAAP = 40.2%; thesis uses non-GAAP trajectory for 40% target — target now achieved
